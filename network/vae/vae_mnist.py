...
def loss_fn(x, x_reconst, mu, log_sigma, beta=0.312):
    # reconst_loss = F.binary_cross_entropy(x_reconst, x, reduction='sum')
    reconst_loss = F.mse_loss(x_reconst, x, reduction='sum')
    kl_loss = -0.5 * torch.sum(1 + 2 * log_sigma - mu.pow(2) - torch.exp(log_sigma*2))
...
class Encoder(nn.Module):
    def __init__(self, num_filters=128,
        ...
        #self.mu_layer = nn.Linear(num_filters * 72, bottleneck_size)
        ...
        #self.logvar_layer = nn.Linear(num_filters * 72, bottleneck_size)
